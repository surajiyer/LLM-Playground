...
from general_utils.llm import get_llm


logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)
chroma_client = chromadb.PersistentClient(
    path=str(Path(__file__).parent.parent / "chroma.db"),
)
...
PROMPT_TEMPLATE = json.dumps({
    ...
    "instructions": [
        ...
        " from transcript`.",
        "Must be minimum 10 points. Add more points if needed and are not similar to"
        " previous points.",
...
def download_transcript(yt_vid_link: str) -> str:
    ...
    try:
        collection.add(
            ...
            metadatas=[{"source": yt_vid_link, "type": "transcript"}],
            ids=[video_id]
        )
    except Exception as e:
    ...
    for i in range(MAX_RETRIES):
        ...
        try:
            for i, t in enumerate(transcript_parts):
                ...
                if (
                    i + 1 < number_of_parts
                    and not GenericResponse.validate_response(response, 'waiting')
                ):
                    retry = True
                    break
...
def main(link: str):
    # download the transcript of the link provided
    transcript = download_transcript(link)

    # summarize the transcript
    summarize_transcript(link, transcript)
...
if __name__ == "__main__":
    ...
    parser.add_argument("link", help="Youtube video link", default="https://www.youtube.com/watch?v=Fkhoc3H2Ois")
